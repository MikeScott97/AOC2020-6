...
namespace AOC2020_6
{
    class Program
    {
        ...
        {
            string rawInput = @"""rypdeiqkbgacnxwumhtozfjvs
...
si""";
            ...
            int totalCount = 0, tempCount;
            List<char> uniques = new List<char>();

            for(int i = 0; i < rows.Length; i++)
            {
                string[] answers = rows[i].Split("\r\n");
                string fullAnswers = string.Join("", answers);
                uniques.Clear();

                for (int individual = 0; individual < answers[0].Length; individual++)
                {
                    tempCount = 0;


                        for (int value = 0; value < fullAnswers.Length; value++)
                        {

                            if (fullAnswers[individual] == fullAnswers[value])
                            {
                                tempCount++;
                    ...
                    if (tempCount == answers.Length)
                    {
                        uniques.Add(fullAnswers[individual]);
                    }
                }
                Console.WriteLine($"{i} = {uniques.Count}");
                totalCount += uniques.Count;
            }
